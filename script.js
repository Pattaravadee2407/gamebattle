...
function veigAttack(){
	...
	if (attackChoice == 1){
		...
		if (hitChance <=5){
			...
			bottomRow.innerHTML += "<br>Dragon hit with you a basic Blast, doing " + dmg + " damage. You now have " + megaHP + "hp.";
			var megaHPBarWidth = (megaHP/100)*300;
			megaManHP.style.width = megaHPBarWidth + "px";
		} else {
			bottomRow.innerHTML += "<br>Dragon missed!";
		}
	} else {
		var hitChance = Math.round(Math.random()*10);
		if (hitChance <=3){
...
function blast(){
	...
	if (hitChance <=7){
		...
		bottomRow.innerHTML = "You hit Battle Boss Dragon with your blast, doing " + dmg + " damage. Dragon now has " + veigarHP + "hp.";
